        ...
        <div className="font-inter">
            ...
            <header className="flex px-8 justify-between items-center">
                <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-0.5">Manage Bank Highlights</p>
                </div>
                <Link to="/admin/highlights/new" className="bg-[#003399] hover:bg-black text-white px-6 py-2 rounded-lg font-bold text-[10px] uppercase tracking-widest transition-all shadow-sm">
                    <i className="fas fa-plus-circle mr-2"></i> Add Highlight
                </Link>
            </header>
            ...
            <div className="px-8 py-4 space-y-6">
                ...
                <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
                    {highlights.length === 0 ? (
                        <div className="p-8 text-center text-gray-400 text-xs font-bold uppercase tracking-widest">No highlights found</div>
                    ) : (
                        <div className="divide-y divide-gray-50">
                            {highlights.map(item => (
                                <div key={item.id} className="p-6 flex items-start justify-between hover:bg-gray-50 transition">
                                    <div className="flex gap-4">
                                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-xl ${item.is_active === '1' || item.is_active === true ? 'bg-yellow-50 text-yellow-600' : 'bg-gray-100 text-gray-400'}`}>
                                            <i className="fas fa-star"></i>
                                        </div>
                                        <div>
                                            <h4 className="text-sm font-bold text-gray-800">{item.title}</h4>
                                            <p className="text-xs text-gray-500 mt-1 max-w-2xl">{item.description}</p>
                                            {!item.is_active && <span className="inline-block mt-2 px-2 py-0.5 bg-gray-100 text-gray-400 text-[10px] font-bold rounded">Inactive</span>}
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <Link to={`/admin/highlights/edit/${item.id}`} className="text-gray-300 hover:text-[#003399] p-2 transition">
