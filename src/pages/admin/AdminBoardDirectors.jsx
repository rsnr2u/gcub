...
const AdminBoardDirectors = ({ isEmbedded = false }) => {
    ...
    const fetchDirectors = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/board-directors`);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/board-directors/delete/${id}`, {
                method: 'POST'
            });
            const data = await response.json();
            if (data.status === 'success') {
                setMessage({ type: 'success', text: 'Director removed successfully.' });
                fetchDirectors();
            }
        } catch (error) {
    ...
    if (loading) return <div className="p-8 text-center text-gray-500 font-inter">Loading Directors...</div>;
    ...
    const headerContent = (
        <header className="flex px-8 py-2 justify-between items-center">
            ...
            >
                <i className="fas fa-plus-circle"></i> Add New Director
            </Link>
        </header>
    );
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {directors.length === 0 ? (
                                    ...
                                    directors.map((director) => (
                                        <tr key={director.id} className="hover:bg-gray-50/50 transition group">
                                            <td className="px-8 py-4">
                                                <div className="w-12 h-12 rounded-full overflow-hidden border border-gray-100 bg-gray-50">
                                                    {director.image_path ? (
                                                        <img src={`${import.meta.env.VITE_BASE_URL}/${director.image_path}`} alt={director.name} className="w-full h-full object-cover" />
                                                    ) : (
                                            ...
                                            <td className="px-8 py-4">
                                                <h3 className="text-sm font-bold text-gray-900">{director.name}</h3>
                                                <p className="text-xs text-gray-500 font-medium mt-0.5">{director.designation}</p>
                                            </td>
                                            <td className="px-8 py-4 service-desc max-w-xs">
                                                <p className="text-xs text-gray-500 truncate">{director.tagline || '-'}</p>
                                            </td>
                                            <td className="px-8 py-4">
                                                <span className="text-xs font-mono font-bold text-gray-600">{director.display_order}</span>
                                            </td>
                                            <td className="px-8 py-4 text-center">
                                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${director.status === 'active' ? 'bg-green-50 text-green-700' :
                                                    director.status === 'inactive' ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-600'
                                                    }`}>
                                                    {director.status}
                                                </span>
                                            </td>
                                            <td className="px-8 py-4">
                                                <div className="flex items-center justify-center gap-2">
                                                    <Link
                                                        to={`/admin/board-directors/edit/${director.id}`}
...
export default AdminBoardDirectors;
