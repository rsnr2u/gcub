...
const AdminDICGCCertificate = () => {
    ...
    const fetchData = async () => {
        try {
            const res = await fetch('http://localhost:8080/api/dicgc');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await fetch('http://localhost:8080/api/dicgc/update', {
                method: 'POST',
                body: uploadData
            });
                ...
                <div className="space-y-2">
                    ...
                    {formData.file_path && (
                        <div className="mt-2 text-xs text-gray-500 px-1">
                            Current File: <a href={`http://localhost:8080/${formData.file_path}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-bold">View Uploaded Certificate</a>
                        </div>
                    )}
                </div>
